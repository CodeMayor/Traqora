...
/**
 * Per-seat async mutex. All check-then-act sequences on a seat (lock,
 * release, select) must run inside `withSeatLock` so concurrent requests
 * are serialized and a seat can never be double-booked by interleaved
 * read-modify-write cycles. The tail of the chain is always cleared so
 * the map does not grow without bound.
 */
const seatMutexes: Map<string, Promise<unknown>> = new Map();

async function withSeatMutex<T>(key: string, fn: () => Promise<T>): Promise<T> {
  const previous = seatMutexes.get(key) ?? Promise.resolve();
  // The chain never rejects: a failing task is caught so later waiters
  // still run, while the caller below re-observes the original error.
  const tail = previous.then(fn);
  // Store an error-swallowed tail so the chain never rejects for waiters.
  const stored = tail.catch(() => undefined);
  seatMutexes.set(key, stored);
  try {
    return await tail;
  } finally {
    // Drop the mutex entry once this task is the (last) tail so the map
    // does not grow without bound. If another request enqueued itself in
    // the meantime it replaced the entry, so only delete when still ours.
    if (seatMutexes.get(key) === stored) {
      seatMutexes.delete(key);
    }
  }
}
...
export class SeatAvailabilityService {
  ...
  async lockSeat(
    ...
    // All check-then-act steps run inside the per-seat mutex so two
    // concurrent lock requests can never both observe the seat as free.
    await withSeatMutex(`${flightId}:${seatNumber}`, async () => {
      // Check if seat already locked. Re-locking by the same booking is
      // idempotent: it refreshes the lock expiry instead of failing.
    ...
    logger.info("Seat locked", { flightId, seatNumber, bookingId });
  }
  ...
  async releaseSeatLock(
    ...
    // Release also runs under the mutex so it cannot race with a concurrent
    // lockSeat for the same seat (e.g. release-after-expiry + re-lock).
    await withSeatMutex(`${flightId}:${seatNumber}`, async () => {
  ...
  async selectSeat(
    ...
    // Seat selection validates availability and persists the booking's seat
    // inside the same per-seat mutex, so a concurrent hold cannot slip in
    // between the availability check and the save.
    await withSeatMutex(`${flightId}:${seatNumber}`, async () => {
...
/** Test-only: clears all in-memory locks and mutex chains. */
export function resetSeatLocksForTesting(): void {
  seatLocks.clear();
  seatMutexes.clear();
}
