...
import { validateRecommendationEvent } from './analyticsEventSchema';
...
/**
 * Records a recommendation analytics event after schema validation
 * (issue #539). Malformed events are dropped and logged — they never
 * enter the pipeline — while valid events keep the same behaviour as
 * before (destination code uppercased, optional reason persisted).
 */
export async function recordRecommendationEvent(input: RecordRecommendationEventInput): Promise<void> {
  const validated = validateRecommendationEvent(input);
  if (!validated.ok) {
    logger.warn('analytics: dropped malformed recommendation event', {
  ...
  const { userId, destinationCode, variant, action, reason } = validated.event;
