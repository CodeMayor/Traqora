...
import { validateAnalyticsEvent, AnalyticsEvent } from './analyticsEventSchema';

/** Ring-buffer cap for recently accepted user-analytics events (observability). */
const RECENT_EVENTS_CAP = 1000;
...
/**
 * Ring buffer of the most recent validated user-analytics events. Keeps the
 * pipeline observable without persisting untrusted payloads.
 */
const recentEvents: AnalyticsEvent[] = [];

/**
 * Ingestion gate for the user-analytics event pipeline (issue #539).
 *
 * Every event is validated against the shared analytics event schema
 * *before* it enters the pipeline. Malformed events are dropped and logged
 * (never silently processed), valid events are normalised (timestamp
 * defaulted, destination code uppercased) and returned to the caller.
 *
 * Idempotent and side-effect free on invalid input — the caller's pipeline
 * state cannot be corrupted by a malformed payload.
 */
export function recordUserAnalyticsEvent(input: unknown): AnalyticsEvent | null {
  const validated = validateAnalyticsEvent(input);
  if (!validated.ok) {
    logger.warn('user-analytics: dropped malformed event', {
  ...
  const event: AnalyticsEvent = {
    ...validated.event,
    destinationCode: validated.event.destinationCode?.toUpperCase(),
    timestamp: validated.event.timestamp ?? new Date().toISOString(),
  };
  ...
  logger.debug('user-analytics: event accepted', { eventType: event.eventType });
  return event;
}

/** Snapshot of recently accepted events (for metrics/monitoring). */
export function getRecentUserAnalyticsEvents(): readonly AnalyticsEvent[] {
  return recentEvents;
}
